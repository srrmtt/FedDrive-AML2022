...
class StyleApplication(object):
  def __init__(self, style):
        self.style = style
        self.style_detector = StyleAugment(0.2)
        self.style_detector.styles.append(style)
  def __call__(self, img, lbl=None):
        return self.style_detector.apply_style(img), lbl
class Resize(object):
    ...
    def __call__(self, img, lbl=None):
        if lbl is not None:
            return cv2.resize(img, (self.size[1], self.size[0])), cv2.resize(lbl, (self.size[1], self.size[0]), interpolation =cv2.INTER_NEAREST)
        else:
